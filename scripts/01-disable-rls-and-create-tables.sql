...
ALTER TABLE IF EXISTS villages DISABLE ROW LEVEL SECURITY;
ALTER TABLE IF EXISTS village_staff DISABLE ROW LEVEL SECURITY;
ALTER TABLE IF EXISTS citizens DISABLE ROW LEVEL SECURITY;
ALTER TABLE IF EXISTS letter_requests DISABLE ROW LEVEL SECURITY;
...
-- Disable RLS on village_staff table
...
-- Disable RLS on letter_requests table
...
-- Disable RLS on system_settings table
...
-- Final verification - show all tables and their row counts
SELECT 
    schemaname,
    tablename,
    tableowner
...
SELECT u.username, u.role, u.status FROM users u WHERE u.username = 'admin';
