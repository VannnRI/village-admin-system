-- Script untuk membuat sistem autentikasi sederhana (FIXED)
...
-- Tambahkan UNIQUE constraint pada username jika belum ada
DO $$ 
BEGIN
    IF NOT EXISTS (
...
-- Tambahkan UNIQUE constraint pada NIK di tabel citizens jika belum ada
DO $$ 
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.table_constraints 
        WHERE constraint_name = 'citizens_nik_key' 
        AND table_name = 'citizens'
    ) THEN
        ALTER TABLE citizens ADD CONSTRAINT citizens_nik_key UNIQUE (nik);
    END IF;
END $$;
...
SELECT 'Script 11-create-simple-auth-system-fixed.sql completed successfully' AS status;
