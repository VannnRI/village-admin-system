...
import { Menu, Home, FileText, LogOut, ShieldCheck } from "lucide-react"
...
export default function MasyarakatLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    if (storedUser) {
      ...
      if (parsedUser.role === "masyarakat") {
        setUser(parsedUser)
      } else {
        localStorage.removeItem("user") // Invalid role for this layout
  ...
  if (!isClient || !user) {
    // Optional: Show a loading spinner or a blank page until client-side check is complete
    return (
  ...
  const navItems = [
    { href: "/masyarakat/dashboard", label: "Dashboard", icon: Home },
    { href: "/masyarakat/request-letter", label: "Ajukan Surat", icon: FileText },
    // Add more navigation items for masyarakat here
  ]
          ...
          <nav className="hidden md:flex items-center space-x-4">
            {navItems.map((item) => (
              <Link
                key={item.label}
                href={item.href}
                className="text-gray-600 dark:text-gray-300 hover:text-blue-600 dark:hover:text-blue-400 transition-colors px-3 py-2 rounded-md text-sm font-medium"
              >
          ...
          <div className="md:hidden">
            <Sheet>
              ...
              <SheetContent side="left">
                <div className="p-4">
                  <Link href="/masyarakat/dashboard" className="flex items-center gap-2 text-lg font-bold mb-6">
